...
describe("When I deposit collateral ERC20 on the Minter dApp, I start to earn HALO rewards.\n\tWhen I withdraw collateral ERC20, I stop earning HALO rewards", function() {
    ...
    it("I earn the correct number of HALO tokens per time interval on depositing collateral ERC20", async() => {
        ...
        const actualUnclaimedHaloRewardBal = parseFloat(ethers.utils.formatEther(await rewardsContract.getUnclaimedMinterLpRewardsByUser(collateralERC20Contract.address, owner.address)));
    ...
    it("I stop earning HALO tokens on withdrawing collateral ERC20", async () => {
        ...
        const actualUnclaimedHaloRewardBal = parseFloat(ethers.utils.formatEther(unclaimedMinterLpRewards2ndAttempt))
    ...
    it("Should have correct amount of HALO token balance", async () => {
        ...
        const actualHaloBal = parseFloat(ethers.utils.formatEther(await haloTokenContract.balanceOf(owner.address)))
...
describe("When I supply liquidity to an AMM, I am able to receive my proportion of HALO rewards. When I remove my AMM stake token from the Rewards contract, I stop earning HALO", function() {
    ...
    it("I earn the correct number of HALO tokens per time interval on depositing LPT", async() => {
        ...
        //const actualUnclaimedHaloPoolRewards = Math.round(ethers.utils.formatEther(await rewardsContract.getUnclaimedPoolRewardsByUserByPool(lpTokenContract.address, owner.address)))
        const actualUnclaimedHaloPoolRewards = ethers.utils.formatEther(await rewardsContract.getUnclaimedPoolRewardsByUserByPool(lpTokenContract.address, owner.address))
